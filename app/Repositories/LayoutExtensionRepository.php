...
class LayoutExtensionRepository implements LayoutExtensionRepositoryInterface
{
    ...
    public function getResolvedExtensionPoints(int $templateId, string $extensionPointName): Collection
    {
        ...
        // 2. 템플릿 오버라이드가 있는 모듈 식별자 추출
        $overriddenModules = $allExtensions
        ...
        // 3. 오버라이드된 모듈/플러그인 확장은 제외하고 반환
        return $allExtensions->filter(function ($extension) use ($overriddenModules) {
            // 템플릿 오버라이드는 항상 포함
            ...
            return ! in_array($extension->source_identifier, $overriddenModules, true);
        })->values();
    }
    ...
    public function getResolvedOverlays(int $templateId, string $layoutName): Collection
    {
        ...
        // 3. 오버라이드된 모듈/플러그인 확장은 제외하고 반환
        return $allOverlays->filter(function ($overlay) use ($overriddenModules) {
            // 템플릿 오버라이드는 항상 포함
            if ($overlay->source_type === LayoutSourceType::Template) {
                return true;
            }

            // 오버라이드된 모듈/플러그인 확장은 제외
            return ! in_array($overlay->source_identifier, $overriddenModules, true);
        })->values();
    }
