...
class AllowedPluginFileTypeTest extends TestCase
{
    /**
    ...
    {
        $rule = new AllowedPluginFileType();

        $jsFiles = ['dist/js/plugin.iife.js', 'app.js', 'script.mjs'];
    ...
    public function test_allows_sourcemap_files(): void
    {
        $rule = new AllowedPluginFileType();
        $validator = Validator::make(
            ['path' => 'dist/js/plugin.iife.js.map'],
            ['path' => $rule]
        );
    ...
    {
        $extensions = AllowedPluginFileType::getAllowedExtensions();
