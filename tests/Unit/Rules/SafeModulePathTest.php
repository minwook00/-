...
class SafeModulePathTest extends TestCase
{
    ...
    {
        parent::setUp();
        $this->basePath = base_path('modules/test-module');
        ...
        // 테스트용 파일 생성
        file_put_contents($this->basePath.'/dist/js/module.iife.js', 'test');
        file_put_contents($this->basePath.'/dist/js/components/Button.js', 'test');
    }
    ...
    {
        $rule = new SafeModulePath($this->basePath);
        $validator = Validator::make(
            ['path' => 'dist/js/module.iife.js'],
            ['path' => $rule]
        );
    ...
    public function test_allows_nested_path(): void
    {
        $rule = new SafeModulePath($this->basePath);
        $validator = Validator::make(
            ['path' => 'dist/js/components/Button.js'],
            ['path' => $rule]
        );
